...
| Attribute           | Description                    | Data Type            | Key Type | Function                                                        | Status   |
| ------------------- | ------------------------------ | -------------------- | -------- | --------------------------------------------------------------- | -------- |
| **CustomerID**      | Customer identification number | `INT`                | **PK**   | Uniquely identifies each customer                               | Required |
| **CustomerName**    | Customer's full name           | `NVARCHAR`           | —        | Displays the customer's name in transactions                    | Required |
| **CustomerDOB**     | Customer's date of birth       | `DATE`               | —        | Supports age analysis and birthday-related services             | Optional |
| **CustomerEmail**   | Customer's email address       | `NVARCHAR`           | —        | Used to send confirmations, promotions, and other notifications | Required |
| **CustomerPhone**   | Customer's phone number        | `NVARCHAR`           | —        | Used for appointment confirmation and customer service          | Required |
| **CustomerAddress** | Customer's address             | `NVARCHAR` / `NTEXT` | —        | Used to identify the service area and deliver information       | Required |
...
| **Species**     | Pet species                    | `NVARCHAR` | —        | Used to classify and determine appropriate care services    | Required |
| **Weight**      | Pet's weight                   | `FLOAT`    | —        | Used for health monitoring and service provision            | Required |
| **PetDOB**      | Pet's date of birth            | `DATE`     | —        | Used to estimate age and support vaccination scheduling     | Required |
| **MedicalNote** | Pet's medical notes            | `NTEXT`    | —        | Records special health conditions or medical considerations | Optional |
...
| **EmployeeName**  | Employee's name                | `NVARCHAR` | —        | Displays the employee's name in the interface and service records | Required |
| **EmployeeDOB**   | Employee's date of birth       | `DATE`     | —        | Supports age and insurance-related management                     | Optional |
| **EmployeeEmail** | Employee's email address       | `NVARCHAR` | —        | Used to send work-related information and schedules               | Required |
| **EmployeePhone** | Employee's phone number        | `NVARCHAR` | —        | Used for calls and messages during work shifts                    | Required |
| **Position**      | Employee's job position        | `NVARCHAR` | —        | Supports task assignment and role-based access                    | Required |
...
| **Category**     | Service category                        | `NVARCHAR` | —        | Groups services for display and filtering                   | Required |
| **Description**  | Detailed service description            | `NTEXT`    | —        | Provides customers with clear information about the service | Optional |
| **Price**        | Service price                           | `DECIMAL`  | —        | Used to calculate service charges during booking            | Required |
| **TargetAnimal** | Animal species eligible for the service | `NVARCHAR` | —        | Helps filter services by animal species                     | Required |
...
| **BookingDate**         | Date when the booking was created | `DATE`     | —        | Tracks booking history                              | Required |
...
| **Comment**      | Feedback content                     | `NTEXT`   | —        | Stores the customer's review or comments            | Optional |
...
| **PaymentDateTime** | Payment date and time         | `DATETIME` | —        | Records when the payment was made                               | Required |
| **Amount**          | Payment amount                | `MONEY`    | —        | Records the amount paid by the customer                         | Required |
| **Bill**            | Payment receipt               | `IMAGE`    | —        | Stores an image of the receipt or a reference to the image file | Required |
...
| **ServiceStatus** | Service status                                      | `VARCHAR` | —          | Tracks the progress of each service within the booking                                                              | Required |
| **ServicePrice**  | Unit service price                                  | `DECIMAL` | —          | Stores the service price at the time of booking rather than dynamically referencing the current price in `SERVICES` | Required |
...
| **GeneralCondition** | Pet's general condition, e.g., healthy or anxious                      | `NVARCHAR` | —        | Records the overall condition observed during the precheck     | Required |
| **SkinCondition**    | Notes on skin, coat, parasites, etc.                                   | `TEXT`     | —        | Provides information relevant to bathing and grooming services | Optional |
| **BehaviorNotes**    | Notes on the pet's behavior, e.g., biting, fear of water, or agitation | `TEXT`     | —        | Alerts employees to behavioral considerations during service   | Optional |
...
| **Status**        | Service execution status: Pending, In Progress, Completed, or Skipped | `VARCHAR`  | —          | Indicates the completion status of each service item                                                     | Required |
| **Result**        | Service result, e.g., completed successfully or not completed         | `NVARCHAR` | —          | Provides a brief summary of the result of each service item                                              | Optional |
| **Notes**         | Additional notes regarding abnormalities or special requirements      | `TEXT`     | —          | Allows employees to record observations or specific difficulties                                         | Optional |
...
| **Status**      | Invoice status, e.g., Issued, Cancelled, or Adjusted | `VARCHAR`  | —        | Tracks the current status of the invoice                                                            | Required |
...
| **Role**       | Employee's role during the precheck | `VARCHAR`  | —          | Specifies the employee's role in the examination process   | Required |
